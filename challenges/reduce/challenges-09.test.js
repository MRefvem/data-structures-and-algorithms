...
const mapCurrentEvents = () => currentEvents.news.map(results => new Event(results));
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let counter = 0;
  let arrLength = arr.reduce((element, value, index) => {
      counter++;
  }, []);
  return counter;
};
...
const returnNames = (arr) => {
  let solution = arr.reduce((characters, value) => {
